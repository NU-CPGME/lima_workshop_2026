...
while (my $vline = <$vin>){
    if ($num_threads_running < $threads){
        ...
        if (0 == $pid){
            #my $tpref = "$$";
            open (my $tsout, ">tstats.$outid.$tpref.txt") or die "ERROR: Can't open tstats.$outid.$tpref.txt for writing: $!\n";
            open (my $thout, ">thash.$outid.$tpref.txt") or die "ERROR: Can't open thash.$outid.$tpref.txt for writing: $!\n";
            my @tsnvs;
            my $in;
            if ($path =~ m/\.gz$/){
    ...
    if ($num_threads_running == $threads){
        ...
        open (my $tin, "<thash.$outid.$tpref.txt") or die "ERROR: Can't open thash.$outid.$tpref.txt: $!\n";
        while (my $tline = <$tin>){
            chomp $tline;
...
foreach my $j (0 .. $#order){
    if ($num_threads_running < $threads){
        ...
        if (0 == $npid) {
            ...
            open (my $pout, ">tproc.$outid.$tpref.txt") or die "ERROR: Can't open tproc.$outid.$tpref.txt ($gen) for writing: $!\n";
            ...
            close ($pout);
            unlink("thash.$outid.$tpref.txt");
            exit;
        }
        $num_threads_running++;
        $npid_hash{$npid} = $gen;
        ## process stats
        open (my $in, "<tstats.$outid.$tpref.txt") or die "ERROR: Can't open tstats.$outid.$tpref.txt: $!\n";
        while (my $line = <$in>){
            chomp $line;
            print $sout "$line\n";
        }
        close ($in);
        unlink("tstats.$outid.$tpref.txt");
    }
    if ($num_threads_running == $threads){
        my $npid = wait;
        my $status = $?;
...
foreach (@order){
    my ($gen, $path, $tpref) = @{$_};
    print $tout "\t$gen";
    open (my $in, "<tproc.$outid.$tpref.txt") or die "ERROR: Can't open tproc.$outid.$tpref.txt ($gen) for reading: $!\n";
    push @handarray, $in;
}
print $tout "\n";
my $in_snvs = 0;
