...
## Changes from original version (v0.1)
## Remove positions without variants after filterning
...
while (my $line = <$tin>){
    ...
    delete $hash{"F"};
    delete $hash{"-"};
    my $basecount = keys %hash;

    unless (1 - ($gapcount/$gencount) < $minfrac or $basecount <= 1){
        $outcount++;
